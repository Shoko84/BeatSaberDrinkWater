...
namespace DrinkWater
{
    class DrinkWaterPanel : MonoBehaviour
    {
        ...
        public void OnMapFinished()
        {
            ...
            if (Plugin.config.Value.EnablePlugin && Plugin.config.Value.EnableByPlaytime && IngameInformationsCounter.Instance.IngameTimeSpent.TotalMinutes >= Plugin.config.Value.PlaytimeBeforeWarning)
            {
                IngameInformationsCounter.Instance.ResetTimeSpent();
                DisplayPanelNeeded = true;
            }
            else if (Plugin.config.Value.EnablePlugin && Plugin.config.Value.EnableByPlaycount && IngameInformationsCounter.Instance.CurrentPlaycount >= Plugin.config.Value.PlaycountBeforeWarning)
            {
        ...
        {
            if (Plugin.config.Value.EnablePlugin)
            {
                ...
                if (Plugin.config.Value.ShowGIFs)
                    StartCoroutine(DisplayGifFromRotation());
                RefreshTextContent();
                StartCoroutine(MakeButtonInteractableDelay(_flowCoordinator.panelController.continueButton, Plugin.config.Value.WaitDuration, 0.1f, "0.0"));
        ...
        private IEnumerator DisplayGifFromRotation()
        {
            if (_flowCoordinator != null)
            {
                ...
                {
                    Logger.log.Error("Exception catched on displaying gif: " + e);
                }
                ...
                if (ratio.x > 1 || ratio.y > 1)
                {
                    ...
                    Logger.log.Debug(">>>>> width: "  + _flowCoordinator.panelController.prefWidth);
                    Logger.log.Debug(">>>>> height: " + _flowCoordinator.panelController.prefHeight);
                }
